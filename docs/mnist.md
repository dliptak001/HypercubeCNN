...
| Batch size | 32 |
| Epochs | 20 (stopped early — plateaued) |
| Threads | 32 |
...
98.1% on MNIST without spatial inductive bias demonstrates that hypercube convolution can learn non-trivial image features from Hamming-distance relationships alone. The 2D spatial structure of the image is not encoded anywhere in the architecture — the network discovers useful patterns purely through the hypercube topology.

This result validates the training pipeline and architecture for deployment on **native hypercube data** (molecular fingerprints, Boolean functions) where the Hamming-distance inductive bias is a structural advantage rather than a handicap.
