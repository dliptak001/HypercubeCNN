...
The ML community considers MNIST solved. A linear classifier gets ~92%. HypercubeCNN's 93.7% at 10K samples (and likely 96-97% at 60K) is well below what a 2-layer MLP achieves (~98%) and far below standard CNNs (~99.3%). Reviewers would see this as evidence the architecture *doesn't work*, not that it does.
...
The architecture is a real contribution. The engineering is clean. But MNIST is the wrong benchmark, and "here's a novel architecture that gets 94% on MNIST" won't get past reviewers. The work becomes interesting the moment it runs on data that belongs on a hypercube.
