...
2. **Conv layers (`HCNN`)** — sparse-vertex convolution using K fixed XOR masks per vertex. Default is nn-only (`use_shell_masks=false`): K = DIM nearest-neighbor single-bit flips, giving best accuracy and 1.6x speedup over full masks. Full masks (`use_shell_masks=true`) add DIM-2 cumulative shell masks from HypercubeRC Reservoir for K = 2*DIM - 2, but require higher LR and converge to a worse optimum. Kernel shape: `[c_out * c_in * K]`.
